...
| Simulator reports replay problems separately from policy deltas | `score-simulator` output includes `current_policy_reproduction`, `candidate_decision_deltas`, `candidate_score_state_deltas`, and `guardrail_or_exemption_deltas`; covered by `trace_reader_buckets_replay_deltas_by_kind`. | Pass |
