...
| Current-policy reproduction passes for a local corpus | `cargo run -p score-simulator -- --input testdata/policy-trace/v1-replay-corpus.jsonl` passed with `reproduction_passes=4`, `reproduction_failures=0`, and `dropped_trace_records=0`. | Pass |
