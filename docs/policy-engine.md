...
or receive-idle state is interpreted only by the adapter that produced it.

To avoid per-peer/per-policy database reads, each poll cycle preloads session and offence-history
rows for the in-scope torrent set into a cycle-scoped `PolicyDataStore` cache. Policy adapters still
...
Pending intents are unique by `torrent_hash + peer_ip + policy_name + offence_number`; peer port is retained as enforcement metadata and updated if the same policy offence is retried after reconnect. Startup replay uses a registry of known policy adapters, including policies currently disabled for new evaluations, so a valid pre-restart intent can still be enforced or retried. Unknown future policy names are retained with an error instead of being discarded.
...
Diagnostics are low-cardinality name/value pairs owned by the policy adapter. At most 8 diagnostics
...
Prometheus policy labels are static labels provided by the policy adapter through the common
interface. Reason-code metric labels are also selected by the adapter from that policy's fixed
reason set, with unknown policy-local reasons collapsed to `other`. This keeps series cardinality
bounded by configured policy adapters rather than dynamic remote data, and it avoids concrete
policy matches in the metrics layer. Score-specific histograms remain score-scoped metrics emitted
by the score adapter; receive-idle currently uses the generic policy counters and diagnostics.
...
`policy.receive_idle.enabled` decide which policy adapters evaluate new peer samples. Replay uses
the compiled registry of known policy adapters rather than only the enabled set, so disabling a
