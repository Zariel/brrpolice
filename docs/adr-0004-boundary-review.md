...
- `PolicyEngine::enabled_peer_policies` and `PolicyEngine::replay_peer_policies` construct the
  compiled policy adapter registry from config.
- Score and receive-idle adapters downcast only the opaque session/evaluation state that they
  created.
...
- Persistence still exposes legacy score-session and score-enforcement helpers. These are called
  from the score adapter as compatibility plumbing for existing `peer_sessions` rows, not from the
...
path. It is contained behind the score adapter today, but should be retired after the rollout window
